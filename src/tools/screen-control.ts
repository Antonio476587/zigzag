...
export class ScreenControlTool implements Tool {
  definition = {
    name: 'screen_control',
    ...
    try {
      ...
      switch (args.action) {
        ...
        case 'right_click':
          ...
          break;
        case 'type':
          if (!args.text) throw new Error('text is required for type action');
          result = await this.performType(platform, args.text);
          break;
        case 'key':
          if (!args.key) throw new Error('key is required for key action');
        ...
        case 'drag':
          ...
          break;
        case 'move_mouse':
          if (!args.x || !args.y) throw new Error('x, y are required for move_mouse action');
  ...
  private async performClick(platform: string, x: number, y: number, button: string): Promise<string> {
    switch (platform) {
      case 'linux':
  ...
  private async performDoubleClick(platform: string, x: number, y: number): Promise<string> {
    switch (platform) {
      case 'linux':
  ...
  private async performType(platform: string, text: string): Promise<string> {
  ...
  private async performKey(platform: string, key: string): Promise<string> {
    switch (platform) {
      case 'linux':
  ...
  private async performScroll(platform: string, direction: string, amount: number, x?: number, y?: number): Promise<string> {
    switch (platform) {
      case 'linux':
  ...
  private async performDrag(platform: string, startX: number, startY: number, endX: number, endY: number): Promise<string> {
    switch (platform) {
      case 'linux':
  ...
  private async performMouseMove(platform: string, x: number, y: number): Promise<string> {
    switch (platform) {
      case 'linux':
  ...
  private getLinuxButton(button: string): number {
    switch (button) {
      case 'left': return 1;
  ...
  private convertToLinuxKey(key: string): string {
  ...
  private convertToMacKey(key: string): string {
  ...
  private convertToWindowsKey(key: string): string {

...
export class FileWatcherTool implements Tool {
  ...
  private async startWatching(args: any): Promise<ToolResult> {
    if (!args.path) {
  ...
  private async stopWatching(args: any): Promise<ToolResult> {
    if (!args.watch_id) {
  ...
  private async getStatus(args: any): Promise<ToolResult> {
    if (args.watch_id) {
